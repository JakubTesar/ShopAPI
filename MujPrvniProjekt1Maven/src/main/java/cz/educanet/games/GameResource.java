...
import cz.educanet.Product;
import cz.educanet.Shop;
...
@Path("games") // localhost:8080/MujPrvniProjektMaven/api/pathname
@Produces(MediaType.APPLICATION_JSON)
public class GameResource {
    public String input = "\"\",\"number\",\"game\",\"release_date\",\"price\",\"owners\",\"developer\",\"publisher\",\"average_playtime\",\"median_playtime\",\"metascore\"\n" +
            "\"1\",\"499\",\"Little Triangle\",\"Jan 20 2017\",\"11.99\",\"0 .. 20000\",\"CottonGame\",\"Dreamoji\",\"0\",\"0\",\"98\"\n" +
    ...
    void start() {
        ...
        for (int i = 1; i < inputSplit.length; i++) {
            ...
            if(!p[4].contains("NA")){
                Games games1 = new Games(p[6].replace("\"", ""), Integer.parseInt(p[10].replace("\"", "")), Double.parseDouble(p[4].replace("\"", "")), p[7].replace("\"", ""), p[3].replace("\"", ""), p[2].replace("\"", ""));
                gamesList.add(games1);
            }
            devs.add(p[6].replace("\"", ""));
            pubs.add(p[7].replace("\"", ""));
    ...
    public Response getDev() {
        start();
        devs.stream().distinct();
        return Response.ok(devs).build();
    }

    @GET
    @Path("publisher")
    public Response getPub() {
        start();
